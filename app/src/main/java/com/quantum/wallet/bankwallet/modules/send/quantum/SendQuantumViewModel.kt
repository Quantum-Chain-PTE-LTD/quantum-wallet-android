...
import com.quantum.wallet.bankwallet.modules.send.SendUiState
import com.quantum.wallet.bankwallet.modules.xrate.XRateService
import io.horizontalsystems.marketkit.models.Token
import java.math.BigDecimal

class SendQuantumViewModel(
    val wallet: Wallet,
    val sendToken: Token,
    val adapter: ISendQuantumAdapter,
    private val xRateService: XRateService,
    private val amountService: SendAmountService,
    private val addressService: SendQuantumAddressService,
    ...
    private fun handleUpdatedAddressState(addressState: SendQuantumAddressService.State) {
        this.addressState = addressState

        emitState()
    }

    fun getSendData(): SendQuantumData? {
        val tmpAmount = amountState.amount ?: return null
        val quantumAddress = addressState.quantumAddress ?: return null

        val transactionData = adapter.getTransactionData(tmpAmount, quantumAddress)

        return SendQuantumData(transactionData)
    }
