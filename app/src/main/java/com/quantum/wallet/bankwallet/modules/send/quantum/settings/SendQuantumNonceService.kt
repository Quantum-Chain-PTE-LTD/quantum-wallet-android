...
import com.quantum.wallet.bankwallet.core.Warning
...
class SendQuantumNonceService(
    ...
    suspend fun start() {
        try {
            latestNonce = quantumKit.getNonce(DefaultBlockParameter.Latest).await()
        } catch (e: Throwable) {
            e.printStackTrace()
        }
    ...
    private suspend fun setRecommended() = withContext(Dispatchers.IO) {
        try {
            val nonce = quantumKit.getNonce(DefaultBlockParameter.Pending).await()
